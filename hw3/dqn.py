...
class QLearner(object):
    ...
    def __init__(
        ...
        print("Double Q-learning is ", double_q)
        ...
        # YOUR CODE HERE
        self.q_source = q_func(img_in=obs_t_float, num_actions=self.num_actions, scope="q_source", reuse=False)
        self.q_target = q_func(img_in=obs_tp1_float, num_actions=self.num_actions, scope="q_target", reuse=False)

        #if not double_q:
        y = self.rew_t_ph + gamma * tf.multiply(tf.reduce_max(self.q_target, axis=1), (1.0 - self.done_mask_ph))
        #else:
        #    q_next_target = q_func(obs_tp1_float, self.num_actions, scope="q_func", reuse=True)
        #    best_q = tf.reduce_sum(self.q_target * tf.one_hot(tf.argmax(q_next_target, axis=1), depth=self.num_actions),
        #                           axis=1)
        #   y = self.rew_t_ph + (1.0 - self.done_mask_ph) * gamma * best_q

        action_one_hot = tf.one_hot(self.act_t_ph, self.num_actions, dtype=tf.float32)
        self.total_error = 1. / 2. * tf.reduce_sum(
            huber_loss(y - tf.reduce_sum(tf.multiply(self.q_source, action_one_hot), axis=1)))

        q_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="q_source")
    ...
    def step_env(self):
        ...
        if random.random() < self.exploration.value(self.t) or not self.model_initialized:
            ...
            q_values = self.session.run(self.q_source, feed_dict={self.obs_t_ph: [obs]})
...
def learn(*args, **kwargs):
    ...
    while not alg.stopping_criterion_met():
        if num_episodes % render_frequency == 0:
            alg.env.render()
            time.sleep(0.025)
