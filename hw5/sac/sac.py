...
class SAC:
    ...
    def _policy_loss_for(self, policy, q_function, q_function2, value_function):
        ...
        q_values = tf.squeeze(q_function(
            [self._observations_ph, on_policy_actions]))

        if q_function2 is not None:
            q_values2 = tf.squeeze(q_function2(
                [self._observations_ph, on_policy_actions]))
        ...
        if not self._reparameterize:
            ...
            policy_loss = tf.reduce_mean(log_pis * tf.stop_gradient(targets))
        else:
            # Problem 1.3.B
            # YOUR CODE HERE
    ...
    def _value_function_loss_for(self, policy, q_function, q_function2, value_function):
        ...
        targets = q_values - self._alpha * log_pis
    ...
    def _q_function_loss_for(self, q_function, target_value_function):
        ...
        # YOUR CODE HERE
        q_values = tf.squeeze(q_function(
            [self._observations_ph, self._actions_ph]), axis=1)
        target_values = tf.squeeze(target_value_function(
            self._next_observations_ph), axis=1)
        targets = self._rewards_ph + self._discount * \
            (1 - self._terminals_ph) * target_values
        return tf.losses.mean_squared_error(q_values, targets)
    ...
    def train(self, sampler, n_epochs=1000):
        ...
        for epoch in range(n_epochs):
            for t in range(self._epoch_length):
                ...
                if self.render and epoch % render_frequency == 0:
                    sampler.env.render()
